...
def run_backtest_quarterly(min_trade_val=2000.0, deadband=0.03):
    # 1. 加载资产清单
    ...
    for i, date in enumerate(test_dates):
        ...
        # 核心：每 22 天（约一个月）重整一次兵马，而不是 10 天
        if i % 22 == 0:
            momentum_data = []
            for sym in symbols:
                if sym == "518880" or sym == "510880": continue 
                series = price_df.loc[:date, sym]
                if len(series) > 22:
                    ret = (series.iloc[-1] - series.iloc[-23]) / series.iloc[-23]
                    momentum_data.append({"sym": sym, "ret": ret})
            
            momentum_data.sort(key=lambda x: x['ret'], reverse=True)
            new_targets = [momentum_data[0]['sym'], momentum_data[1]['sym']]
...
if __name__ == "__main__":
    val, trades, fees = run_backtest_quarterly(2000.0, 0.03)
    print(f"--- 🚀 SkyNet '月度重整版' 回测 (2000元门槛 / 3%死区) ---")
