...
def run_backtest_sweet_spot(min_trade_val=1500.0, deadband=0.025):
    # 1. 加载资产清单
    ...
    for i, date in enumerate(test_dates):
        ...
        # 核心：每 10 天扫描一次市场动能，决定 Top 2 进攻标的
        if i % 10 == 0:
            momentum_data = []
            for sym in symbols:
                if sym == "518880": continue # 黄金是盾
                if sym == "510880": continue # 红利是盾
                series = price_df.loc[:date, sym]
                if len(series) > 10:
                    ret = (series.iloc[-1] - series.iloc[-11]) / series.iloc[-11]
                    momentum_data.append({"sym": sym, "ret": ret})
            
            momentum_data.sort(key=lambda x: x['ret'], reverse=True)
            top_2 = momentum_data[:2]
        ...
        for s, target_w in target_weights.items():
            ...
            # 甜蜜点：偏离 > 2.5% 且 交易额 > 1500 元
            # 止损逻辑：如果 target_w == 0，强制清仓
...
if __name__ == "__main__":
    val, trades, fees = run_backtest_sweet_spot(1500.0, 0.025)
    print(f"--- 🚀 SkyNet '黄金平衡' 回测 (1500元门槛 / 2.5%死区) ---")
